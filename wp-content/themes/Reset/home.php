...
<div id="ChronBottom" class="inner">
	...
	<?php
		...
		while(($query->have_posts()) && !(get_post_type($post->ID) == "video")){
		$query->the_post(); 
		$poet[] = get_the_ID(); //this uses awkward showcase
	?>
			<div class="showcase-slide">
				<div class="showcase-content">
					<div <?php post_class('showcase-content-wrapper'); ?>>
						...
						<script>
							$('[title="Fair Game"]').attr("href", "/fair-game/");
						</script>
	...
	<div id="ChronBottomNews" class="pop">
		...
		<?php
			$query = new WP_Query(array( 'post_type' => 'news', 'news_importance' => 'section_slider', 'posts_per_page' => 1, 'post__not_in' => $poet ) );
		...
		<?php
			...
			$query = new WP_Query(array( 'post_type' => 'news', 'posts_per_page' => 3, 'post__not_in' => $poet ) );
	...
	<div id="ChronBottomSports" class="pop">
		...
		<article <?php post_class('first-article'); ?>>
		<?php
			$query = new WP_Query(array( 'post_type' => 'sports', 'sports_importance' => 'section_slider', 'posts_per_page' => 1 ) );
			if($query->have_posts()) { $query->the_post();
			$poet[] = get_the_ID();
			$wp_query->in_the_loop = true;
		?>
	...
	<div id="ChronBottomExtra" class="pop">
		...
		<div class="important">
			<div class="first uppercase sans-serif2 font11 bold">
				<a href="http://www.hwchronicle.com/opinion/get-in-touch-get-involved/">LETTER FROM THE EDITORS-IN-CHIEF</a>
			</div>
	...
	<div id="ChronBottomAE" class="pop">
		<h4 class="usatoday ae"><a href="<?php echo site_url(); ?>/ae/" class="section-link">A&E</a></h4>
		<div class="annoyingbar moveable"><?php the_issue_link($current_month); echo first_word(get_formal_issue_month($current_month)); ?> Issue</a></div>
	<?php
	...
	?>
		...
		<div class="two">
	<?php 
		$query = new WP_Query(array( 'post_type' => 'ae', 'posts_per_page' => 3, 'post__not_in' => $poet ) );
		while($query->have_posts()) { $query->the_post();
	?>
			<article>
				<div class="font14 bold"><?php the_full_title(); ?></div>
			</article>
	<?php
		}
	?>
	...
	<div id="ChronBottomMoreNews" class="pop">
		<h4><a href="<?php echo site_url(); ?>/news/" class="section-link">
			More News
		</a></h4><ul>
		<?php
	...
	<div id="ChronBottomMoreSports" class="pop">
		<h4><a href="<?php echo site_url(); ?>/sports/" class="section-link">
			More Sports
		</a></h4><ul>
	<?php
	...
	?>
		<li><article>
			<div class="first left bold font13 line-height">
				<?php the_full_title(); ?>
			</div>
			<div class="second right red mini">
				<?php the_checked_date(); ?>
