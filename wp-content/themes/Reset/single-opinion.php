...
<?php 
	...
	$is_guestcolumn = is_object_in_term($post->ID, 'opinion_type', 'guest_column');
...
?>
	...
	<div id="SingleTop" >
		...
		if(!$issue_month){
			...
			generic_echo_top($top_object);
		}elseif(!$page_num || $is_guestcolumn){
			$top_object = generic_opinion_top($issue_month, $post->ID);
			generic_echo_top($top_object);
		}elseif($is_column){
			...
			<div id="<?php the_ID(); ?>" <?php post_class(); ?>>
				...
				<div class="<?= $is_editorial ? 'editorial' : 'non-editorial' ?>">
					<h1 class="entry-title">
						...
						}elseif($is_guestcolumn){
							echo "<p class='title-indicator'>Guest Column:</p> ";
						}
						?>
						<?php the_title(); ?>
					</h1>
					...
					<?php } ?>
					<div class="meta left">
						<?php if(!$is_editorial && !$is_letter && !$is_guestcolumn){?>
						...
						<?php }elseif($is_guestcolumn){ ?>
						<p class="opinion-author"><?php echo get_post_meta(get_the_ID(), 'letter_from', true); ?></p>
						<?php } ?>
